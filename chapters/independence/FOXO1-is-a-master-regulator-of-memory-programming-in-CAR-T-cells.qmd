...
TL;DR: Single-cell experiments of this design are effectively not worth the time and money because small *p*-values are almost guaranteed **even if there is no true difference in expression level**. That is, we are extremely unlikely to reliably learn anything.
...
Better than TL;DR: This design will result in extremely small *p*-values for very small, and possibly biologically meaningless, effect sizes. When this happens, researchers are likely to pursue research paths with trivial consequences for mouse health.
