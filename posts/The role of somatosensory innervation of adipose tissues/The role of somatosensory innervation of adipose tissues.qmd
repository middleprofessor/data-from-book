...
## Whoops! The published figure has data that is backwards and upside down

1. The researchers coding of the surgery levels (YFP, Cre) in the Excel file is the reverse of that in original Fig 3g plot. This affects only the direction of the difference and the directions given in GraphPad prism post-hoc tables in the archived Excel file are correct for the data given in that file. This also of course effects the interpretation of the experiment!
2. The 6-OHDA data seems to be the reflection about the y = 0 axis -- that is it is upside down and shifted downwards.
