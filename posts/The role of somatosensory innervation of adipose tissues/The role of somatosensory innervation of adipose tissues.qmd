...
categories: ["linear mixed model", "random intercept", "split plot", "ggplot", "data flag"]
...
## Improving on the authors: adjusting all 6 responses using FDR
...
## What's going on with the direction of the effect in Fig 3g?

![Fig 3g ANOVA table from Excel file](../../figs/The role of somatosensory innervation of adipose tissues/fig3g.png)

Compare my figure with the published figure.

Compared to my figure built from the archived data, the data in Fig3g is reflected about some horizontal axis, that is, the data are upside down, so the apparent direction of the differences is exactly opposite that of the archived data in the Excel file. The GraphPad prism contrasts tables in the Excel file match the archived data in the Excel file, so the data in the figure would seem to be wrong, unless I'm missing something. This doesn't matter for the p-values but since directions of the differences are exactly opposite, the interpretation of the results is exactly opposite.

But, I think this is what we want. The top matrix of archived values are $\Delta C_q$ values, which decrease as expression levels increase. So to get the effects in the correct direction, we need to multiple the $\Delta C_q$ values by -1.

Let's reflect the gene expression values by multiplying them by -1 and replot:
...
Okay - this replicates the plot but what is the justification for reflecting the data and reversing the direction of the difference?
